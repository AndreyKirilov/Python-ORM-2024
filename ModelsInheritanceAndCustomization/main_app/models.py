...
class BaseReservation(models.Model):
    ...
    def calculate_total_cost(self):
        return round(self.room.price_per_night * (self.end_date - self.start_date).days, 2)

    @property
    def is_available(self):
        reservations_for_room = self.__class__.objects.filter(room=self.room,
                                                              end_date__gte=self.start_date,
                                                              start_date__lte=self.end_date)
        return not reservations_for_room.exists()
